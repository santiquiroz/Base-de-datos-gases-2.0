...
public class DataBase extends Conexion {
    ...
    private String setCampos2(String SQL, ArrayList map) {
        int lenght = map.size();
        for (int i=0; i<lenght;i++) {
            ArrayList key = (ArrayList) map.get(i);
            String key2 = (String) key.get(0);
            SQL += key2;
            if (i != lenght-1) {
                SQL += " , ";
            }
    ...
    private String setValores2(String SQL, ArrayList map) {
        int lenght = map.size();
        for (int i=0; i<lenght;i++) {
            ArrayList key = (ArrayList) map.get(i);
            String value = (String) key.get(1);
            if (value == null) {
                SQL += null;
            } else {
                SQL += "'" + value + "'";
